<header id="mainHeader" class="fixed top-0 left-0 w-full z-50 md:z-60">
  <nav class="bg-transparent backdrop-blur-sm border-b border-white/40 text-white shadow-none transition-all duration-300">
    <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">

      <!-- Logo -->
      <a href="/" class="flex items-center gap-5 font-extrabold text-2xl text-white hover:scale-105 transform transition-all duration-300">
      ...
      <ul class="hidden md:flex items-center gap-8 text-sm font-bold text-white">
        ...
        <li><a href="{{ route('landing.tecnologias') }}" class="hover:text-beigeCalido transition-all duration-300 hover:scale-105">Tecnologías</a></li>
        <li><a href="{{ route('landing.equipo')}}" class="hover:text-beigeCalido transition-all duration-300 hover:scale-105">Equipo</a></li>
        ...
        <li class="relative" id="desktopClinicas">
          ...
          <ul id="desktopClinicasList" class="absolute left-1/2 -translate-x-1/2 mt-3 w-60 bg-white/10 backdrop-blur-md border border-white/30 text-white rounded-3xl shadow-2xl opacity-0 pointer-events-none transition-all duration-300 transform -translate-y-2">
            <li><a href="{{ route('landing.santafe') }}" class="block px-5 py-3 hover:bg-white/20 rounded-xl transition-all duration-300">Santa Fe</a></li>
            <li><a href="{{ route('landing.pedregal') }}" class="block px-5 py-3 hover:bg-white/20 rounded-xl transition-all duration-300">Pedregal</a></li>
            <li><a href="{{ route('landing.queretaro') }}" class="block px-5 py-3 hover:bg-white/20 rounded-xl transition-all duration-300">Querétaro</a></li>
          </ul>
        </li>
      ...
      <!-- Botón blanco -->
      <a href="#contacto" class="hidden md:inline-block border border-white text-white px-6 py-3 font-bold shadow-lg hover:bg-white hover:text-black transition-transform transform hover:-translate-y-1 hover:scale-105">
        Agenda tu Diagnóstico
      </a>

      <!-- Botón menú móvil -->
      <button id="menuBtn" class="md:hidden text-3xl text-white hover:scale-110 transition-transform relative z-50">☰</button>
    </div>

    <!-- Menú móvil -->
    <div id="mobileMenu" class="fixed top-0 left-0 w-full bg-white backdrop-blur-xl duration-300 z-50 scale-0 opacity-0 overflow-auto">
      <div class="max-w-7xl mx-auto px-6 pt-32 flex flex-col items-center space-y-6 text-2xl text-black h-auto justify-start">
        ...
        <div class="relative w-full max-w-xs">
          <button class="accordion-btn w-full py-3 bg-beigeCalido text-verdeOscuro rounded-3xl font-bold shadow-lg hover:scale-105 transition-transform">
          ...
          <div class="accordion-content max-h-0 overflow-hidden transition-all duration-300">
            <a href="{{ route('landing.santafe') }}" class="block py-2 pl-6 hover:text-beigeCalido transition-all duration-300">Santa Fe</a>
            <a href="{{ route('landing.pedregal') }}" class="block py-2 pl-6 hover:text-beigeCalido transition-all duration-300">Pedregal</a>
            <a href="{{ route('landing.queretaro') }}" class="block py-2 pl-6 hover:text-beigeCalido transition-all duration-300">Querétaro</a>
          </div>
        </div>
...
<script>
  ...
  window.addEventListener('scroll', () => {
    ...
    if (scrollY > 0) {
      ...
      buttons.forEach(btn => {btn.classList.remove('text-white','border-white'); btn.classList.add('text-gray-800','border-gray-700');});
      ...
      buttons.forEach(btn => {btn.classList.remove('text-gray-800','border-gray-700'); btn.classList.add('text-white','border-white');});
